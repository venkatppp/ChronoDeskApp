...
export interface IntelligenceRepository {
  ...
  getLatestWorkspaceHealth(workspaceId: string): Promise<WorkspaceHealth | null>;
  getWorkspaceHealthHistory(workspaceId: string, days: number): Promise<WorkspaceHealth[]>;
  getWorkspaceRecommendations(workspaceId: string): Promise<Recommendation[]>;
  getCategoryRecommendations(workspaceId: string, category: string): Promise<Recommendation[]>;
  getPriorityRecommendations(workspaceId: string, minPriority: string): Promise<Recommendation[]>;
}
...
export class TauriIntelligenceRepository implements IntelligenceRepository {
  ...
  async getLatestWorkspaceHealth(workspaceId: string): Promise<WorkspaceHealth | null> {
    return invoke<WorkspaceHealth | null>("get_latest_workspace_health", { workspaceId });
  }

  async getWorkspaceHealthHistory(workspaceId: string, days: number): Promise<WorkspaceHealth[]> {
    return invoke<WorkspaceHealth[]>("get_workspace_health_history", { workspaceId, days });
  }

  async getWorkspaceRecommendations(workspaceId: string): Promise<Recommendation[]> {
    return invoke<Recommendation[]>("get_workspace_recommendations", { workspaceId });
  }

  async getCategoryRecommendations(workspaceId: string, category: string): Promise<Recommendation[]> {
    return invoke<Recommendation[]>("get_category_recommendations", { workspaceId, category });
  }

  async getPriorityRecommendations(workspaceId: string, minPriority: string): Promise<Recommendation[]> {
