...
export interface GlassInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, "size"> {
  ...
  size?: "sm" | "md" | "lg";
  /** Click handler for read-only/search-trigger fields. */
  onClick?: (e: React.MouseEvent<HTMLInputElement>) => void;
}

/** Shared glass text field — one input vocabulary for the whole app. */
export const GlassInput = forwardRef<HTMLInputElement, GlassInputProps>(
  ({ className, icon, size = "md", onClick, readOnly, ...props }, ref) => (
    <span className={cn("relative block w-full", className)}>
      ...
      <input
        ...
        className={cn(
          ...
          "hover:shadow-[inset_0_1px_2px_rgba(0,0,0,0.2),0_0_0_1px_rgba(255,255,255,0.08)]",
          size === "sm" && "h-7 px-3 text-xs",
          size === "md" && "h-9 px-3.5 text-sm",
          size === "lg" && "h-11 px-4 text-[15px]",
          icon && (size === "lg" ? "pl-11" : "pl-10"),
          readOnly && "cursor-pointer bg-(--color-surface-hover)/50",
